...
if ty1
  ...
  if idx
    check = 0;
    warn(a,idx,' Vr is over its max limit.')
  end
  idx = find(vr < vrmin(ty1));
  if idx
    check = 0;
    warn(a,idx,' Vr is under its min limit.')
  end
end
...
if ty2
  ...
  if idx
    Tf(idx) = 0.1;
    a.con(ty2(idx),8) = 0.1;
    warn(a,idx,[' Tf cannot be zero. ', ...
	    'Default value Tf = 0.1 will be used.'])
  end
  idx = find(Ta <= 0);
  if idx
    a.con(ty2(idx),6) = 0.1;
    warn(a,idx,[' Ta cannot be zero. ', ...
  ...
  if idx
    check = 0;
    warn(a,idx,' Vr1 is over its max limit.')
  end
  idx = find(Ce < vrmin(ty2));
  if idx
    check = 0;
    warn(a,idx,' Vr1 is under its min limit.')
  end
end
...
if ty3
  ...
  if idx
    a.con(ty3(idx),6) = 0.1;
    warn(a,idx,[' T2 cannot be zero. ', ...
	    'Default value T2 = 0.1 will be used.'])
  end
  idx = find(vf((ty3)) > vrmax(ty3));
  if idx
    check = 0;
    warn(a,idx,' Vf is over its max limit.')
  end
  idx = find(vf(ty3) < vrmin(ty3));
  if idx
    check = 0;
    warn(a,idx,' Vf is under its min limit.')
  end
end
